...
const init = async (guilds: Array<string>) => {
...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isChatInputCommand()) {
        ...
        if (!voiceChannel) {
            interaction.reply({
                content: 'You are not in a voice channel dummy',
            });
            return;
        }
        ...
        if (interaction.commandName === Commands.meme) {
            ...
            if (input) {
                ...
                if (inputSound) {
                    ...
                    interaction.reply({
                        content: `You dummy McIdiot, '${input}' doesn't exist`,
                    });
                    return;
                }
            }
            ...
            interaction.reply({
                content: `Playing '${sound.split('.')[0]}' good sir`,
            });
